...
The Agent inbox is the only queue. Every Agent message uses `Agent.steer()`: an idle target starts a turn, while a running target claims it at the nearest step boundary. The browser `subagent.prompt` Remote separately carries `delivery: 'queue' | 'steer' | 'interrupt'` through the same internal admission path. Queue opens a later FIFO turn, Steer retains the Agent loop's best-effort nearest-step behavior, and Interrupt preserves pending work while canceling a still-running turn before appending a fresh FIFO turn. Successful delivery returns the accepted `MessageId`; the existing inbox splice events remain the message-lifecycle observations, and the continuation layer defines no second queue.
