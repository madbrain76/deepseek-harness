...
Run `dsh --profile web` to open an interactive browser GUI with chat, model and settings management, and session history. It uses the same model access, tools, and safety defaults as other dsh surfaces. Startup prints an authenticated URL and normally opens it in the default browser; SSH sessions and `--no-open` leave the URL for manual opening. You can change the port, allow extra hosts, or explicitly bind every interface. Choose this package for interactive browser work; use `dsh-headless` for one-shot command-line tasks.
...
- **Binding all interfaces exposes the agent runtime to the LAN** — `--host 0.0.0.0` is an explicit operator choice. Every reachable client can attempt the authenticated browser handshake against a surface that has shell and code-execution tools, so use it only on a trusted network and add DNS authorities with `--trusted-host` when browsers use a hostname.
