...
运行 `dsh --profile web`，打开提供聊天、模型与设置管理以及会话历史的交互式浏览器 GUI。它使用与其他 dsh 表层相同的模型访问、工具与安全默认值。启动时会打印带认证信息的 URL，通常还会在默认浏览器中打开；SSH 会话和 `--no-open` 会保留该 URL，供你手动打开。你可以更改端口、允许额外主机，或显式绑定所有网络接口。需要在浏览器中交互式工作时选择本包；一次性的命令行任务应使用 `dsh-headless`。
...
- **绑定所有接口会向 LAN 暴露 agent 运行时**——`--host 0.0.0.0` 是运维方的显式选择。每个可达客户端都能尝试针对具备 shell 与代码执行工具的表层完成带认证的浏览器握手，因此只能在可信网络中使用；浏览器使用主机名时，还需通过 `--trusted-host` 添加对应 DNS authority。
