...
Prompt 投递接受 Queue、Steer 与 Interrupt。Interrupt 在最后的在线 Agent cutoff 之前完成验证与附件准入；如果 Agent 仍在运行，控制器保留 inbox、取消活跃 activity，并在两个操作之间不让出执行权地把已准入消息追加到 `nextTurn`。Agent loop 会把 abort 后的唤醒输入归类为 next-turn 工作，因此消息只在取消达到静止后运行。重复的 Interrupt prompt 保留 FIFO 顺序；如果 prompt 到达 cutoff 时 Agent 已空闲，则按 Queue 处理。cutoff 前的拒绝绝不会取消活跃 activity。
