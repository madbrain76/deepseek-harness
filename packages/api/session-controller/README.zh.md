...
提示词投递支持 Queue、Steer 与 Interrupt。Interrupt 会在最终 live-Agent cutoff 前完成验证和附件准入；如果 Agent 仍在运行，控制器会保留其 inbox，取消当前 activity，并且不在这些操作之间让出执行权地把已准入消息追加到 `nextTurn`。Agent loop 将中止后的唤醒输入归类为下一轮工作，因此消息只会在取消完全结束后运行。重复的 Interrupt 提示词保持 FIFO 顺序；在 Agent 已空闲后才到达 cutoff 的提示词会按 Queue 处理。cutoff 前的拒绝绝不会取消当前 activity。
