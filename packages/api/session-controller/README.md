...
Prompt delivery accepts Queue, Steer, and Interrupt. Interrupt completes validation and attachment admission before its final live-Agent cutoff; if the Agent is still running, the controller preserves its inbox, cancels the active activity, and appends the admitted message to `nextTurn` without yielding between those operations. The Agent loop classifies waking input after an abort as next-turn work, so the message runs only after cancellation quiesces. Repeated Interrupt prompts retain FIFO order, and a prompt that reaches the cutoff after the Agent becomes idle behaves as Queue. A rejection before the cutoff never cancels the active activity.
